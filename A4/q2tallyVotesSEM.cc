#include "q2tallyVotes.cc"
#include <uCobegin.h>

TallyVotes::Tour TallyVotes::vote( unsigned int id, TallyVotes::Tour ballot ) {

    lock.P(); // enter critical block
    try {
        // register vote
        result += ballot == TallyVotes::Tour::Picture ? +1 : -1;
        // print vote
        printer.print( id, Voter::States::Vote, ballot );
    } _Finally { lock.V(); }

    if( barrier.waiters()+1 < barrier.total() ) { // if blocking, print blocking
        printer.print( id, Voter::States::Block, barrier.waiters()+1 );
        barrier.block();
        printer.print( id, Voter::States::Unblock, barrier.waiters() );
    }
    else { // if last, release the Paco
        printer.print( id, Voter::States::Complete );
        res = result > 0 ? TallyVotes::Tour::Picture : TallyVotes::Tour::Statue;
        result = 0;
        barrier.block();
    }

    return res;
}

void uMain::main() {
    L1: {
        uint v, g, seed;
        if( !checkInput( argv, argc, g, v, seed ) ) break L1;

        myrand.seed(seed);

        Printer p( v );
        TallyVotes tb( g, p );

        COFOR( i, 0, v, Voter a( i, tb, p ); );
    }
}
